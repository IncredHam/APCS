  ...
  **/       
   private boolean isPlaced(int row, int col)
   {
      ...
      else
      {
         for(int k = col; k < N; k++)
         {
            if(locationIsOK(row, k))
            {
               addQueen(row, k);
               if(isPlaced(row + 1, col))
                  return true;
               else
                  removeQueen(row, k);
            }
  ...
  /** Verify that another queen can't attack this location.
   ...
   private boolean locationIsOK(int r, int c)
   {
      for(int k = 0; k < r; k++)
      {
         if(board[k][c].getBackground() == Color.RED)
            return false;
      }
      for(int k = 0; r - k >= 0; k++)
      {
         if(c + k < N && board[r - k][c + k].getBackground() == Color.RED)
            return false;
      }
      for(int k = 0; r - k >= 0; k++)
      {
         if(c - k >= 0 && board[r - k][c - k].getBackground() == Color.RED)
            return false;
      }
      return true;
   }
